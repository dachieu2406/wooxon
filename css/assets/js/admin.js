...
(function($) {
    ...
    $(document).ready(function(){
        $('#wooxon_theme_options-section_documentation_row a').attr('target','_blank');
        $(document)
            // Upload Single Image
                ...
                file_frame.on( 'select', function() {
                    ...
                    selection.map( function( attachment ) {
                        ...
                        if ( attachment.id ) {
                            ...
                            $preview.append( '<div class="wooxon-image" data-attachment_id="' + attachment.id + '"><img src="' + attachment_image + '" /><a href="#" class="delete" title="Delete image">Delete</a></div>' );
                        }
                    });
            ...
            .on( 'click', '.wooxon-image .delete', function(e){
                e.preventDefault();
                $( this ).closest( 'div.wooxon-image' ).remove();
                var $parent = $(this).closest('.gallery-field');
                var $image_gallery_ids = $('#' + $parent.find('.button').data('id'));
                var attachment_ids = '';

                $parent.find( 'div.wooxon-image' ).css( 'cursor', 'default' ).each( function() {
            ...
            $('.box-option .gallery-field .gallery-preview').sortable({
                ...
                helper: 'clone',
                opacity: 0.65,
                placeholder: 'wooxon-metabox-sortable-placeholder',
                ...
                update: function( event, ui ) {
                    ...
                    $preview.find( 'div.wooxon-image' ).css( 'cursor', 'default' ).each( function() {
                        var attachment_id = $( this ).attr( 'data-attachment_id' );
                        attachment_ids = attachment_ids + attachment_id + ',';
                    });
	...
	var delSidebar = '<a class="delete-sidebar handlediv" href="javascript:void(0)" title="Delete Sidebar"><span class="dashicons dashicons-trash"></span></a>';
	
	jQuery('.sidebar-piko_custom_sidebar').find('.sidebar-name button').after(delSidebar);      
	
	$(document).on('click', '.delete-sidebar', function() {            
